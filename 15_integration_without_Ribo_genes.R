...
if (file.exists(file.path(path.to.15.output, "s8_output", "integrated_BrainMet_dataset.output.s8.rds")) == FALSE){
  ...
  remove.genes <- unlist(lapply(all_genes, function(x){
  ...
  s.obj.integrated <- s8.integration.and.clustering_V5(s.obj = s.obj, 
                                                       save.RDS.s8 = TRUE,
                                                       path.to.output = path.to.15.output,
                                                       use.sctransform = TRUE,
                                                       num.PCA = num.PCA,
  ...
  print(sprintf(
    "Data exists at %s",
    file.path(path.to.15.output, "s8_output", "integrated_BrainMet_dataset.output.s8.rds")
  ))
  s.obj.integrated <- readRDS(file.path(path.to.15.output, "s8_output", "integrated_BrainMet_dataset.output.s8.rds"))
}
