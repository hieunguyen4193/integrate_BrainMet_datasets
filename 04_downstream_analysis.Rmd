---
title: "Downstream analysis on integrated dataset, `r basename(params$path.to.sobj)`"
author:
...
# path.to.sobj <- file.path(path.to.03.output, "s8_output", sprintf("%s.output.s8.rds", PROJECT))
...
path.to.04.output <- file.path(path.to.main.output, "04_output_test", object.save.name)
...
for (reduction.method in c("harmony")){
  ...
  p <- DimPlot(object = s.obj, reduction = sprintf("%s_UMAP", reduction.method), label = TRUE, label.box = TRUE, repel = TRUE, group.by = sprintf("%s.cluster.0.5", reduction.method))  
  print(p)
  cat("\n \n")
}
...
min.pct <- 0.5
Idents(s.obj) <- sprintf("%s.cluster.0.5", reduction.method)
...
for (cluster.id in sort(unique(cluster.markers$cluster))){
  ...
  cat(sprintf("### Cluster %s \n", cluster.id))
  tmp.cluster.markers <- subset(cluster.markers, cluster.markers$cluster == cluster.id) %>% arrange(desc(avg_log2FC))
  p <- DotPlot(object = s.obj, features = head(tmp.cluster.markers, 9)$gene, group.by = sprintf("%s.cluster.0.5", reduction.method))  
  print(p)
  cat("\n \n")
}
```
...
for (cluster.id in sort(unique(cluster.markers$cluster))){
  ...
  cat(sprintf("### Cluster %s \n", cluster.id))
  tmp.cluster.markers <- subset(cluster.markers, cluster.markers$cluster == cluster.id) %>% arrange(desc(avg_log2FC))
  p <- VlnPlot(object = s.obj, features = head(tmp.cluster.markers, 9)$gene, slot = "data",  group.by = sprintf("%s.cluster.0.5", reduction.method), pt.size = 0)  
  print(p)
  cat("\n \n")
}
```
...
pct.table <- count.cell.in.cluster.wider %>% column_to_rownames(sprintf("%s.cluster.0.5", reduction.method))
...
```{r results='asis', echo=FALSE, message=FALSE, warning=FALSE, fig.width=14, fig.height=10}
pct.table.sample.longer <- pct.table.sample %>% rownames_to_column(sprintf("%s.cluster.0.5", reduction.method)) %>% 
...
pct.table.cluster.longer <- pct.table.cluster %>% rownames_to_column("name") %>% 
  pivot_longer(!name, names_to = sprintf("%s.cluster.0.5", reduction.method), values_to = "Freq")
...
for (gene.id in tk.gene.set){
  ...
  if (file.exists(file.path(path.to.04.output, "TK_genes", sprintf("violin_plot_group_%s_clusters.png", gene.id))) == FALSE){
    p.clusters <- VlnPlot(object = s.obj, group.by = sprintf("%s.cluster.0.5", reduction.method), features = c(gene.id), split.by = "group", slot = "data", pt.size = 0)  
...
if (file.exists(file.path(path.to.04.output, "diff.markers_BrainMet_vs_Control_Epilepsy.rds")) == FALSE){
  ...
  count.cells.in.clusters <- table(s.obj$group, s.obj$harmony.cluster.0.5) %>% as.data.frame()
  for (cluster.id in unique(s.obj$harmony.cluster.0.5)){
    ...
    tmp.s.obj <- subset(s.obj, harmony.cluster.0.5 == cluster.id)
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
path.to.project.src <- "/media/hieunguyen/HNSD01/src/UKK/src/BrainMet/scRNAseq/20240601"
...
meta.data <- s.obj@meta.data %>% rownames_to_column("barcode") %>%
  rowwise() %>%
  mutate(prediction3 = ifelse(nrow(subset(annotationdf, annotationdf$cluster.id == harmony.cluster.0.5)) != 0, 
                              subset(annotationdf, annotationdf$cluster.id == harmony.cluster.0.5)$annotation,
                              "None")) %>%
  column_to_rownames("barcode")
meta.data <- meta.data[row.names(s.obj@meta.data),]
