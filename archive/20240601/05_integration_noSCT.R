...
# s.obj <- subset(s.obj, cells = sample(all.cells, 10000))
...
if (file.exists(file.path(path.to.05.output, "s8_output", sprintf("%s.output.s8.rds", PROJECT))) == FALSE){
  s.obj.integrated <- s8.integration.and.clustering_V5(s.obj = subset(s.obj, name %in% keep.samples), 
                                                       save.RDS.s8 = TRUE,
                                                       path.to.output = path.to.05.output,
  ...
  s.obj.integrated <- readRDS(file.path(path.to.05.output, "s8_output", sprintf("%s.output.s8.rds", PROJECT)))
}

min.pct <- 0.5
Idents(s.obj.integrated) <- "cca.cluster.0.5"
if (file.exists(file.path(path.to.05.output, sprintf("DE_cluster_marker_genes_min_pct_%s.rds", min.pct))) == FALSE){
  ...
  saveRDS(cluster.markers, file.path(path.to.05.output, sprintf("DE_cluster_marker_genes_min_pct_%s.rds", min.pct)))
} else {
  cluster.markers <- readRDS(file.path(path.to.05.output, sprintf("DE_cluster_marker_genes_min_pct_%s.rds", min.pct)))
}
