...
to.integrate.samples <- read.csv("/media/hieunguyen/HNSD01/src/UKK/src/BrainMet/scRNAseq/20240601/samples_to_integrated_20240513.csv")
...
all.dataset.name <- c("GSE186344", "GSE200218", "GSE234832", "GSE140393")
for (dataset.name in all.dataset.name){
  all.sample.ids <- subset(to.integrate.samples, to.integrate.samples$Dataset == dataset.name)$Sample
  
  for (input.sample.id in all.sample.ids){
    sample.id <- sprintf("%s_%s", dataset.name, input.sample.id)
    my_random_seed <- 42
    set.seed(my_random_seed)
    
    # __________VDJ DATA ANYLYSIS PIPELINE__________
    PROJECT <- "BrainMet_20240615"
    ...
    path.to.main.input <- "/media/hieunguyen/HNHD01/data/UKK/BrainMet/preprocess_raw_data"
    ...
    path2input <- file.path(path.to.main.input, sample.id)
    ...
    path.to.output <- file.path(path.to.main.output, sample.id)
    ...
    save.outdir <- "/media/hieunguyen/HNHD01/outdir/BrainMet_rerun_without_SCT_20240615"
