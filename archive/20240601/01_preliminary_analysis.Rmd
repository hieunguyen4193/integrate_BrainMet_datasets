...
path.to.main.output <- file.path(outdir, PROJECT, output.version, dataset.name, sample.id)
...
DoHeatmap(object = s.obj, features = tk.gene.set) + scale_fill_gradient()
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=20, fig.height=12}
dir.create(file.path(path.to.01.output, "TK_genes"), showWarnings = FALSE, recursive = TRUE)
for (gene.id in intersect(tk.gene.set, row.names(s.obj))){
  if (file.exists(file.path(path.to.01.output, "TK_genes", sprintf("violin_plot_group_%s_predicted_celltype.png", gene.id))) == FALSE){
    p.celltype <- VlnPlot(object = s.obj, group.by = "prediction2", features = c(gene.id), pt.size = 0)
    ggsave(plot = p.celltype, filename = sprintf("violin_plot_group_%s_predicted_celltype.png", gene.id), device = "png", width = 14, height = 10, path = file.path(path.to.01.output, "TK_genes"))
  }
  if (file.exists(file.path(path.to.01.output, "TK_genes", sprintf("violin_plot_group_%s_clusters.png", gene.id))) == FALSE){
    p.clusters <- VlnPlot(object = s.obj, group.by = "seurat_clusters", features = c(gene.id), slot = "data", pt.size = 0)  
    ggsave(plot = p.clusters, filename = sprintf("violin_plot_group_%s_clusters.png", gene.id), device = "png", width = 14, height = 10, path = file.path(path.to.01.output, "TK_genes"))
  }
}

```

# Check TK gene expressions
