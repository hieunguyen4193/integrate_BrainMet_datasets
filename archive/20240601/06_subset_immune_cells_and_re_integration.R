...
path.to.03.output <- file.path(path.to.main.output, "03_output_test")
path.to.06.output <- file.path(path.to.main.output, sprintf("06_output_test_%s", cluster.resolution))
...
if (file.exists(file.path(path.to.06.output, "s8_output", sprintf("%s.output.s8.rds", PROJECT))) == FALSE){
  ...
  s.obj.integrated <- readRDS(file.path(path.to.06.output, "s8_output", sprintf("%s.output.s8.rds", PROJECT)))
}

min.pct <- 0.5
Idents(s.obj.integrated) <- "cca.cluster.0.5"
if (file.exists(file.path(path.to.06.output, sprintf("DE_cluster_marker_genes_min_pct_%s.rds", min.pct))) == FALSE){
  ...
  saveRDS(cluster.markers, file.path(path.to.06.output, sprintf("DE_cluster_marker_genes_min_pct_%s.rds", min.pct)))
} else {
  cluster.markers <- readRDS(file.path(path.to.06.output, sprintf("DE_cluster_marker_genes_min_pct_%s.rds", min.pct)))
}
