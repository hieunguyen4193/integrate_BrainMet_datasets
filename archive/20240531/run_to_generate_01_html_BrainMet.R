...
fetch_all_folders <- basename(Sys.glob(file.path(input.outdir, "Brain*")))
project.type <- "Brain"
...
for (PROJECT in all.PROJECTS){
  ...
  for (dataset.name in all.datasets){
    all.samples <- to_vec (for (item in fetch_all_folders) if(grepl(sprintf("%s_%s", PROJECT, dataset.name), item) == TRUE ) str_replace(item, sprintf("%s_%s_", PROJECT, dataset.name), ""))
    for (sample.id in all.samples){
      ...
      if (file.exists(file.path(path.to.save.html, save.html.filename)) == FALSE){
        ...
        dir.create(file.path(input.outdir, sprintf("%s_%s_%s", PROJECT, dataset.name, sample.id), sprintf("%s_%s", dataset.name, sample.id), "data_analysis"))
        system(sprintf("mv  %s/* %s", 
                       file.path(outdir, PROJECT, dataset.name, sample.id),
                       file.path(input.outdir, sprintf("%s_%s_%s", PROJECT, dataset.name, sample.id), sprintf("%s_%s", dataset.name, sample.id), "data_analysis")))
