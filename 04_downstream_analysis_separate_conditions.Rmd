...
# path.to.sobj <- "/media/hieunguyen/HNSD01/outdir/BrainMet_SeuratV5/integrate_BrainMet_datasets/integrated_v0.2/09_output/s8_output/integrated_BrainMet_dataset.output.s8.rds"
# cluster.resolution <- 0.5
# outputdir <- "/media/hieunguyen/HNSD01/outdir/BrainMet_SeuratV5/integrate_BrainMet_datasets/integrated_v0.3/"

path.to.04.output <- outputdir
dir.create(path.to.04.output, showWarnings = FALSE, recursive = TRUE)

logdf <- data.frame(input.s.obj = c(path.to.sobj), cluster.resolution = c(cluster.resolution), outputdir = c(outputdir))
writexl::write_xlsx(logdf, file.path(path.to.04.output, "log.xlsx"))
...
# 
# if (file.exists(file.path(path.to.04.output, "finished_saving_TK_genes.csv")) == FALSE){
...
#     }
#   }  
#   write.csv(data.frame(status = c("finished saving TK genes")), file.path(path.to.04.output, "finished_saving_TK_genes.csv"))
# }
...
# DefaultAssay(s.obj) <- "SCT"
# 
# if (file.exists(file.path(path.to.04.output, "finished_saving_cell_marker_genes.csv")) == FALSE){
...
#     }  
#   }
#   write.csv(data.frame(status = c("finished saving cell marker genes")), file.path(path.to.04.output, "finished_saving_cell_marker_genes.csv"))
# }

```

# Marker genes for Microglia and MDM

## UMAP
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
FeaturePlot(object = s.obj, reduction = sprintf("%s_UMAP", reduction.method), label = TRUE, repel = TRUE, features = c("ITGA4", "THBD", "ICAM1"))
```

```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
VlnPlot(object = s.obj, features = c("ITGA4", "THBD", "ICAM1"), ncol = 2)
```
